...
**Work order now lives in `docs/BACKLOG.md`** - read that for what to do next, in order. `docs/ROADMAP.md` stays the deep explanation of what is broken and why; the backlog is only about priority. Top item is confirming `VITE_USDA_API_KEY` is actually set in the Cloudflare Pages dashboard (blocked on Kevin - sessions can't reach the deployed site through the egress policy). Top *unblocked* item is the phone-width toolbar, for which Kevin has approved a "More" disclosure.

Three things blocked on Kevin: whether `VITE_USDA_API_KEY` is set in the Cloudflare Pages dashboard (a local `.env` does **not** reach Cloudflare's build runners, and `VITE_*` vars are inlined at build time, so a dashboard change needs a fresh deploy to take effect - `docs/ARCHITECTURE.md` records this having bitten Kevin twice, once for this exact key); live-testing the nutrition matching, which needs the environment's network policy widened to allow `api.nal.usda.gov` and `world.openfoodfacts.org` (the API key alone is not enough - the hosts are currently refused by egress policy, which is also why a session cannot verify the first item); and the dietician's July 31 plan revision, not yet supplied, to diff against the June 19 text hard-coded in `App.jsx`.
