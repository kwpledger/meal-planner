...
- **Cloudflare Pages scopes environment variables separately for Production and Preview** - and **KV bindings the same way**. A value or binding set on only one scope produces a preview that behaves nothing like production. The two differ in when they take effect: `VITE_*` variables are inlined at *build* time so a dashboard edit does nothing until a redeploy, while a KV binding is read at *request* time.
