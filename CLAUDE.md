...
- **The shared design system is attached and typography is adopted.** `@kwpledger/design` is pinned at **v0.2.0** (`docs/DESIGN-SYSTEM.md` covers the wiring). Lora display / Hanken Grotesk body are live; **every colour token is still unadopted** - the board paints itself with ~200 hard-coded Tailwind neutrals plus indigo/amber/sky/rose/green. `docs/BACKLOG.md` item 2 carries the remaining order. Pin a tag, never a branch: a design system that moves under a consumer is how a shared system becomes a liability. The repo is **`kwpledger-design`** (not `kwpledger-designsystem`, which appears in older notes and is wrong).
