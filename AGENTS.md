...
**Full reasoning for all thirteen is in `docs/DECISIONS.md`.** Kept here only
where not knowing it in the first four minutes gets something broken:

- **Never silently overwrite a number Kevin is looking at.** Nutrition lookups
  are advisory; the dietician's figures are the baseline. Match data may be
  written automatically because it changes nothing visible — calories and macros
  need Recompute -> preview diff -> Apply.
...
- **The sync endpoint is unauthenticated on purpose.** A shared-secret header
  *cannot* work in a browser-only SPA — it would ship in the bundle. The 1 MB
  body cap and shape validation are the real guards. Don't "fix" this.
- **Sync is manual push/pull, never automatic**, and runs on Cloudflare KV
  rather than Supabase. Both were settled with evidence; don't re-litigate
  either.
- **Five traps live in `.claude/rules/` and load when you open the file they
  belong to.** Editing anything in `src/` or `functions/` without having seen
  its rule? Read it first.
...
- **This file has a word budget: 1,350 soft, 1,850 hard.** Past soft, weigh
  each addition and look for something to move to `docs/`, which carries no
  limit. Past hard, move something *now*. Full rules and reasoning:
