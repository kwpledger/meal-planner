...
- **The app is a consumer of `@kwpledger/design`, pinned at v0.5.1.** Typography, surfaces, borders and text are adopted; accent, categorical and status are not. Tokens layer palette -> semantic -> *domain*, and **a domain token must never reach past the semantic layer to a raw palette value** - the one rule the layering exists to enforce. Defining a semantic value locally is allowed with a stated reason (§10.3): `--surface-sunken` does, pending an upstream report. Colours are *compatible*, not *harmonized*. The repo is **`kwpledger-design`** (`kwpledger-designsystem` in older notes is wrong). **Read `docs/DESIGN-SYSTEM.md` and `docs/BACKLOG.md` item 2 before any colour work** - the traps there have each already cost a session.
